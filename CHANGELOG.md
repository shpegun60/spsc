...
This patch release closes the full post-v3.0.2 paranoid-audit backlog: a raw
`latest` C-array publish defect, exception swallowing and accidental O(N^2)
accessor complexity in runtime-shaped `buffer_pool`, Windows `min`/`max`
macro compatibility across all public headers, checked-path shadow
preservation for `try_consume()` and single RAII guards, and early
const/volatile/raw-array payload contracts. It does not change the v3
default-policy model or public state layouts.
...
[Unreleased]: https://github.com/shpegun60/spsc/compare/v3.0.3...HEAD
[3.0.3]: https://github.com/shpegun60/spsc/compare/v3.0.2...v3.0.3
